...
std::function<void(std::string, LogLevel)> Logger::StandardErrorOutput = [](const std::string& message, LogLevel)
{
    std::cerr << message << '\n';
};


std::function<void(std::string, LogLevel)> Logger::StandardOutput = [](const std::string& message, LogLevel)
{
    std::cout << message << '\n';
};
...
void Logger::Log(LogLevel level, const std::string& message, const char* file, int line)
{
    ...
    {
        if (level & m_output[i].sensitivity)
        {
            ...
            ss << message;
            m_output[i].callback(ss.str(), level);
        }
    }
}

void Logger::AddOutput(LogLevel sensitivity, std::function<void(const std::string& message, LogLevel level)> callback)
{
    m_output.emplace_back(callback, sensitivity);
}
