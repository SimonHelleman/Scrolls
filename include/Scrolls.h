...
#include <vector>
...
#ifdef SCROLLS_USE_LOGGER_MACROS
    #ifndef SCROLLS_DISABLE_DEBUG_LOG
        #define LOG_DEBUG(message) scrolls::Logger::Get().LogDebug(message)
        #define LOG_DEBUG_FL(message) scrolls::Logger::Get().LogDebug(message, __FILE__, __LINE__)
    #else
    ...
    #define INFO(message) scrolls::Logger::Get().LogInfo(message)
    #define INFO_FL(message) scrolls::Logger::Get().LogInfo(message, __FILE__, __LINE__)
    #define WARN(message) scrolls::Logger::Get().LogWarning(message);
    #define WARN_FL(message) scrolls::Logger::Get().LogWarning(message, __FILE__, __LINE__)
    #define ERROR(message) scrolls::Logger::Get().LogError(message);
    #define ERROR_FL(message) scrolls::Logger::Get().LogError(message, __FILE__, __LINE__)
#endif
...
public:
    ...
    void AddOutput(LogLevel sensitivity, std::function<void(const std::string& message, LogLevel level)> callback);
